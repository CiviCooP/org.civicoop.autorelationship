...
class CRM_Autorelationshipnl_Creator {
   ...
   * @param type $target_contact_ids
   * @param type $relationship_type_id
   */
  protected function endOldRelationships($objAddress, $target_contact_ids, $relationship_type_id) {
    ...
    if (isset($result['values']) && is_array($result['values'])) {
      foreach($result['values'] as $relationship) {
        ...
        /*
         * Check for end date, if end date is already set and this 
         */
        if ($relationship['custom_10'] == $objAddress->id) {
